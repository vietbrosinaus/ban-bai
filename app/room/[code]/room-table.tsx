...
import { ArrowLeft, Check, Copy, Crown, Crosshair, Eye, EyeOff, Hand, Heart, Layers3, Link2, LoaderCircle, LogOut, Minus, Plus, Redo2, RefreshCw, RotateCcw, Shield, Shuffle, Sparkles, Trash2, Undo2, Users, Wifi, WifiOff } from "lucide-react";
...
import type { HiddenGeneral, PlayingCard, PublicPlayerBoard, PublicRoom, SelectedGeneral, Suit } from "@/lib/game";
...
export default function RoomTable() {
  ...
  async function moveSelected(destination: string, targetPlayerId?: string) {
    if (!selectedCardId) return;
    const result = await sendAction("move-card", { cardId: selectedCardId, destination, targetPlayerId });
    ...
    <main className="room-shell">
      ...
      <section className={`felt-table ${dragOver ? "is-drop-target" : ""}`} onDragEnter={(event) => { event.preventDefault(); setDragOver(true); }} onDragOver={(event) => event.preventDefault()} onDragLeave={(event) => { const nextTarget = event.relatedTarget; if (!(nextTarget instanceof Node) || !event.currentTarget.contains(nextTarget)) setDragOver(false); }} onDrop={(event) => { event.preventDefault(); setDragOver(false); const cardId = event.dataTransfer.getData("text/card-id"); if (cardId) void sendAction("play", { cardId }); }}>
        ...
        <div className="table-center">
          <div className="deck-area">
            ...
            {isTamQuocSat && <button type="button" className="discard-pile-button" onClick={() => setDiscardOpen(true)}><Trash2 /><span>Discard</span><b>{room?.discard.length ?? 0}</b></button>}
          </div>
        ...
        <aside className="table-controls">
          ...
          {isTamQuocSat && <Button variant="outline" disabled={isActing || !room?.table.length} onClick={() => void sendAction("clear-table")}><Trash2 className={pendingAction === "clear-table" ? "sync-spinner" : ""} /> Clear played cards</Button>}
      ...
      <section className={`hand-dock ${isTamQuocSat ? "tam-hand-dock" : ""}`}>
        <div className="hand-meta">
          ...
          {!isTamQuocSat && <p>Drag or tap a card to play it</p>}
        ...
        {isTamQuocSat && selectedCard && (
          <div className="card-action-bar" role="toolbar" aria-label={`Actions for ${selectedCard.name}`}>
            <span><b>{selectedCard.name}</b><small>{selectedCard.rank}{suitSymbol[selectedCard.suit]}</small></span>
            <Button size="sm" onClick={() => void moveSelected("table")} disabled={isActing}><Hand /> Play</Button>
            <Button size="sm" variant="outline" onClick={() => void moveSelected("equipment")} disabled={isActing}><Shield /> Equip</Button>
            <Button size="sm" variant="outline" onClick={() => void moveSelected("judging")} disabled={isActing}><Sparkles /> Judge</Button>
