...
const playingCardVariants = cva(
  "relative aspect-[5/7] shrink-0 select-none overflow-hidden rounded-[0.55em] bg-card-face bg-cover bg-center shadow-[0_0.5em_1.1em_rgba(0,0,0,0.32)] ring-1 ring-card-edge",
  {
    variants: {
      size: {
        xs: "w-11 text-[0.5rem]",
        sm: "w-14 text-[0.6rem]",
        md: "w-[4.625rem] text-[0.7rem]",
        lg: "w-24 text-[0.8rem]",
      },
      face: {
        up: "",
    ...
    >
      {faceDown && (
        <span className="absolute inset-0 grid place-items-center">
          <i className="grid aspect-square w-[45%] rotate-45 place-items-center border border-gilt font-serif text-[0.9em] font-extrabold text-gilt not-italic">
      ...
      {!faceDown && card && !general && (
        <>
          <CardCorner rank={card.rank} suit={card.suit} className="top-[0.4em] left-[0.45em]" />
          <CardCorner rank={card.rank} suit={card.suit} className="right-[0.45em] bottom-[0.4em]" />
          {!card.art && <SuitMark suit={card.suit} className="absolute inset-0 grid place-items-center text-[2.4em] opacity-30" />}
        </>
      )}
      {!faceDown && general && card && (
        <>
          {card.faction && <span className={cn("absolute inset-x-0 top-0 h-[0.4em]", FACTION_FILL[card.faction])} />}
          <span className="absolute inset-x-0 bottom-0 grid gap-px bg-gradient-to-t from-black/90 to-transparent px-[0.5em] pt-[1.6em] pb-[0.4em] text-left text-white">
            <b className="truncate text-[0.92em] leading-tight font-semibold">{card.name}</b>
            <small className="flex items-center gap-[0.4em] overflow-hidden text-[0.62em] font-bold tracking-wide whitespace-nowrap text-white/80">
